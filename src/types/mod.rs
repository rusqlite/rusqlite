...
mod test {
    ...
    fn test_mismatched_types() {
        ...
        assert!(is_invalid_column_type(row.get_checked::<Index, c_int>(0).err().unwrap()));
        assert!(is_invalid_column_type(row.get_checked::<Index, c_int>(0).err().unwrap()));
        assert!(is_invalid_column_type(row.get_checked::<Index, i64>(0).err().unwrap()));
        assert!(is_invalid_column_type(row.get_checked::<Index, c_double>(0).err().unwrap()));
        assert!(is_invalid_column_type(row.get_checked::<Index, String>(0).err().unwrap()));
        assert!(is_invalid_column_type(row.get_checked::<Index, time::Timespec>(0).err().unwrap()));
        assert!(is_invalid_column_type(row.get_checked::<Index, Option<c_int>>(0).err().unwrap()));

        // 1 is actually a text (String)
        assert!(is_invalid_column_type(row.get_checked::<Index, c_int>(1).err().unwrap()));
        assert!(is_invalid_column_type(row.get_checked::<Index, i64>(1).err().unwrap()));
        assert!(is_invalid_column_type(row.get_checked::<Index, c_double>(1).err().unwrap()));
        assert!(is_invalid_column_type(row.get_checked::<Index, Vec<u8>>(1).err().unwrap()));
        assert!(is_invalid_column_type(row.get_checked::<Index, Option<c_int>>(1).err().unwrap()));

        // 2 is actually an integer
        assert!(is_invalid_column_type(row.get_checked::<Index, String>(2).err().unwrap()));
        assert!(is_invalid_column_type(row.get_checked::<Index, Vec<u8>>(2).err().unwrap()));
        assert!(is_invalid_column_type(row.get_checked::<Index, Option<String>>(2).err().unwrap()));

        // 3 is actually a float (c_double)
        assert!(is_invalid_column_type(row.get_checked::<Index, c_int>(3).err().unwrap()));
        assert!(is_invalid_column_type(row.get_checked::<Index, i64>(3).err().unwrap()));
        assert!(is_invalid_column_type(row.get_checked::<Index, String>(3).err().unwrap()));
        assert!(is_invalid_column_type(row.get_checked::<Index, Vec<u8>>(3).err().unwrap()));
        assert!(is_invalid_column_type(row.get_checked::<Index, Option<c_int>>(3).err().unwrap()));

        // 4 is actually NULL
        assert!(is_invalid_column_type(row.get_checked::<Index, c_int>(4).err().unwrap()));
        assert!(is_invalid_column_type(row.get_checked::<Index, i64>(4).err().unwrap()));
        assert!(is_invalid_column_type(row.get_checked::<Index, c_double>(4).err().unwrap()));
        assert!(is_invalid_column_type(row.get_checked::<Index, String>(4).err().unwrap()));
        assert!(is_invalid_column_type(row.get_checked::<Index, Vec<u8>>(4).err().unwrap()));
        assert!(is_invalid_column_type(row.get_checked::<Index, time::Timespec>(4).err().unwrap()));
    }
    ...
    fn test_dynamic_type() {
        ...
        assert_eq!(Value::Blob(vec![1, 2]),
                   row.get_checked::<Index, Value>(0).unwrap());
        assert_eq!(Value::Text(String::from("text")),
                   row.get_checked::<Index, Value>(1).unwrap());
        assert_eq!(Value::Integer(1), row.get_checked::<Index, Value>(2).unwrap());
        match row.get_checked::<Index, Value>(3).unwrap() {
            Value::Real(val) => assert!((1.5 - val).abs() < EPSILON),
            x => panic!("Invalid Value {:?}", x),
        }
        assert_eq!(Value::Null, row.get_checked::<Index, Value>(4).unwrap());
    }
}
